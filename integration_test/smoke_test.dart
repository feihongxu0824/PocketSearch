...
void main() {
  ...
  setUpAll(() async {
    ...
    final imgNet = mnn.Interpreter.fromBuffer(imageModel.buffer.asUint8List());
    imgNet.setSessionMode(mnn.SessionMode.Session_Backend_Auto);
    final imgSession = imgNet.createSession(
      config: mnn.ScheduleConfig.create(type: mnn.ForwardType.MNN_FORWARD_AUTO),
    );
    final imgInput = imgNet.getSessionInput(imgSession);
    final imgOutput = imgNet.getSessionOutput(imgSession);
    // ignore: avoid_print
    print('=== IMAGE MODEL NATIVE TENSORS ===');
    // ignore: avoid_print
    print('input:  shape=${imgInput?.shape} type=${imgInput?.type}');
    // ignore: avoid_print
    print('output: shape=${imgOutput?.shape} type=${imgOutput?.type}');
    final allImageOutputs = imgNet.getSessionOutputAll(imgSession);
    // ignore: avoid_print
    print(
      'image outputs (${allImageOutputs.length}): '
      '${allImageOutputs.entries.map((e) => "${e.key}=${e.value.shape}").join(", ")}',
    );

    // Also dump text encoder tensors.
    final textNet = mnn.Interpreter.fromBuffer(textModel.buffer.asUint8List());
    textNet.setSessionMode(mnn.SessionMode.Session_Backend_Auto);
    final textSession = textNet.createSession(
      config: mnn.ScheduleConfig.create(type: mnn.ForwardType.MNN_FORWARD_AUTO),
    );
    final textInput = textNet.getSessionInput(textSession);
    final textOutput = textNet.getSessionOutput(textSession);
    final allTextOutputs = textNet.getSessionOutputAll(textSession);
    // ignore: avoid_print
    print('TEXT input: shape=${textInput?.shape} type=${textInput?.type}');
    // ignore: avoid_print
    print(
      'TEXT output(default): shape=${textOutput?.shape} type=${textOutput?.type}',
    );
    // ignore: avoid_print
    print(
      'text outputs (${allTextOutputs.length}): '
      '${allTextOutputs.entries.map((e) => "${e.key}=${e.value.shape}").join(", ")}',
    );
    tokenizer = Tokenizer();
    ...
    await store.initialize(
      '${appDir.path}/zvec_smoke_${DateTime.now().millisecondsSinceEpoch}',
    );
  });
  ...
  testWidgets('Different images produce DIFFERENT image embeddings', (
    ...
    final red = (await rootBundle.load(
      'assets/test_images/solid_red.png',
    )).buffer.asUint8List();
    final green = (await rootBundle.load(
      'assets/test_images/solid_green.png',
    )).buffer.asUint8List();
    final blue = (await rootBundle.load(
      'assets/test_images/solid_blue.png',
    )).buffer.asUint8List();
    ...
    // ignore: avoid_print
    print(
      'image sims: red/green=${rg.toStringAsFixed(4)} '
  ...
  testWidgets('REAL photos produce DIFFERENT image embeddings', (tester) async {
    final a = (await rootBundle.load(
      'assets/test_images/real/photo_a.jpg',
    )).buffer.asUint8List();
    final b = (await rootBundle.load(
      'assets/test_images/real/photo_b.jpg',
    )).buffer.asUint8List();
    ...
    // ignore: avoid_print
    print(
      'ea[0..7] = ${ea.sublist(0, 8).map((v) => v.toStringAsFixed(4)).join(",")}',
    );
    // ignore: avoid_print
    print(
      'eb[0..7] = ${eb.sublist(0, 8).map((v) => v.toStringAsFixed(4)).join(",")}',
    );
    expect(
      sim,
      lessThan(0.95),
      reason:
          'Two very different real photos should not produce near-identical embeddings',
    );
  });
  ...
  testWidgets('Diagnostic: cross-modal cosine distribution on real photos', (
    ...
    for (final q in [
      ...
      // ignore: avoid_print
      print(
        'text="$q"  photo_a(shelf)=${sa.toStringAsFixed(4)}  '
  ...
  testWidgets('Cross-modal: text color queries rank matching color image #1', (
    ...
    for (final color in ['red', 'green', 'blue']) {
      ...
      // ignore: avoid_print
      print(
        'text "$color" -> ${scored.map((e) => "${e.key}=${e.value.toStringAsFixed(3)}").join(", ")}',
      );
      expect(
        scored.first.key,
        color,
        reason:
            'text "$color" must match $color image, got ${scored.first.key}',
      );
    }
  });
  ...
  testWidgets('End-to-end: image + text embedding + vector store', (
    ...
    store.insert(photoId: 'red_photo', vector: imgEmb, photoPath: '/t/red.png');
    store.optimize();
  ...
  testWidgets(
    'Image encoding benchmark (used to estimate full-gallery indexing time)',
    (tester) async {
      ...
      // ignore: avoid_print
      print(
        'Image encoder avg: ${avgMs.toStringAsFixed(1)} ms/call '
